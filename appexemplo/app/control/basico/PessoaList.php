...
class PessoaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $row1 = $this->form->addFields([new TLabel("id:", null, '14px', null)],[$idpessoa]);
        $row2 = $this->form->addFields([new TLabel("Nome:", null, '14px', null)],[$nome]);
        $row3 = $this->form->addFields([new TLabel("CPF:", null, '14px', null)],[$cpf]);
        $row4 = $this->form->addFields([new TLabel("Data de nascimento:", null, '14px', null)],[$dat_nascimento]);
        $row5 = $this->form->addFields([new TLabel("municipio nascimento:", null, '14px', null)],[$id_municipio_nascimento]);
        ...
        $column_idpessoa = new TDataGridColumn('idpessoa', "id", 'center' , '70px');
        $column_nome = new TDataGridColumn('nome', "Nome", 'left');
        $column_cpf = new TDataGridColumn('cpf', "CPF", 'left');
        $column_dat_nascimento = new TDataGridColumn('dat_nascimento', "Data nascimento", 'left');
        $column_municipio_nascimento_nom_municipio = new TDataGridColumn('municipio_nascimento->nom_municipio', "municipio nascimento", 'left');
        ...
        $this->datagrid->addColumn($column_dat_nascimento);
        $this->datagrid->addColumn($column_municipio_nascimento_nom_municipio);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Básico","Pessoa"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->dat_nascimento) AND ( (is_scalar($data->dat_nascimento) AND $data->dat_nascimento !== '') OR (is_array($data->dat_nascimento) AND (!empty($data->dat_nascimento)) )) )
        {

            $filters[] = new TFilter('dat_nascimento', '=', $data->dat_nascimento);// create the filter 
        }

        if (isset($data->id_municipio_nascimento) AND ( (is_scalar($data->id_municipio_nascimento) AND $data->id_municipio_nascimento !== '') OR (is_array($data->id_municipio_nascimento) AND (!empty($data->id_municipio_nascimento)) )) )
        {

            $filters[] = new TFilter('id_municipio_nascimento', '=', $data->id_municipio_nascimento);// create the filter 
        }
